...
var level_info = {
	"RHYTHM_HELL" = {
		# Dados do nível (muito longo, mantido como string para consistência)
		...
		"music": load("res://cenas/fases/fase_boto/music/Rhythm Hell.wav")
	}
}

# ----------------------------------------------------------------------
# FUNÇÃO _ready (NÃO FAZ MAIS NADA ALÉM DE CONECTAR SINAIS)
# ----------------------------------------------------------------------

func _ready():
	# Conexão de sinais de edição, se estiver no modo de edição
	if in_edit_mode:
		Signals.KeyListenerPress.connect(KeyListenerPress)
	
	# O restante do código de inicialização foi movido para start_level()

# ----------------------------------------------------------------------
# NOVA FUNÇÃO DE INICIALIZAÇÃO (CHAMADA APÓS O TUTORIAL)
# ----------------------------------------------------------------------

func start_level():
	# ✅ Passo 1: Inicia a música APÓS o tutorial fechar e o jogo despausar
	$MusicPlayer.stream = level_info.get(current_level_name).get("music")
	$MusicPlayer.play()
	
	# ✅ Passo 2: Inicia a geração das setas
	if not in_edit_mode:
...
func KeyListenerPress(button_name: String, array_num: int):
	# Usado apenas em edit_mode, mas mantém a lógica para evitar crash
...
func SpawnFallingKey(button_name: String, delay: float):
	# Usa o 'delay' da música para agendar a criação da seta
